...
void BallotList::loadFile(const std::string fileName) throw (std::invalid_argument){
    ...
    if (read.is_open()){
        ...
        while (!read.eof()){
            read >> sState >> sDate >> sPerson >> sRegion;
            sState.pop_back();
            sDate.pop_back();
            sPerson.pop_back();
            std::cout << sState << sDate << sPerson << sRegion;
...
void BallotList::insertBallot(const std::string state, const std::string person,
    ...
    Ballot bal;
    [state, person, date, region, &bal](State tState, Candidate tPerson)->void{
